...
window.initializePlayground = async (opts) => {
	...
	async function handleLanguageChange() {
		...
		try {
			const response = await fetch(`${LANGUAGE_BASE_URL}/default.scm`);
			if (!response.ok) {
		...
		try {
			const response = await fetch(`${LANGUAGE_BASE_URL}/example.cook`);
			if (!response.ok) {
				throw new Error(`Failed to load example.cook: ${response.status}`);
			}
